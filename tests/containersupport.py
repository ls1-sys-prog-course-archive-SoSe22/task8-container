...
def ensure_nix() -> None:
    if os.getuid() == 0:
        warn("The script was run as uid 0. This task should not require root due to the user namespaces. Drop to nobody user")

    try:
        run(["nix-build", "--version"])
    except OSError as e:
        fail(f"Could not run `nix-build --version` ({e}). Did you install nix correctly?")
...
class BuildDir:
    def __init__(self) -> None:
        ...
        res = run(["nix-build", "--keep-failed", "--builders", "", str(test_root().joinpath("nix"))], check=False, stderr=subprocess.PIPE)

        print(res.stderr)
        match = re.search(r"keeping build directory '([^']+)'", res.stderr)
        if not match:
            fail("could not find build directory of failing nix build in the output of `nix-build`")
